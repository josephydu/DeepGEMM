            ...
            while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
                launch_k_iterations([&](int k_iter, auto type) {
                    ...
                    for (uint32_t s = 0; s < kNumInnerStages; ++ s) {
                        ...
                        constexpr uint32_t group_size = 2048; // 4096 / 2
                        const uint32_t group_offset = group_idx * group_size;
                        tma_copy(&tensor_map_scales_b, reinterpret_cast<uint64_t*>(&full_barrier),
                                 smem_scales_b[s], n_block_idx * BLOCK_N, 
                                 scheduler.get_global_idx<false>(group_size, 1, k_idx / BLOCK_K, 0) + group_offset);
